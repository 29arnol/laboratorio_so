	...
	if ($filas === 0 or $consultaBusqueda == "") {
		...
		while($resultados = mysqli_fetch_array($consulta)) {
			...
			if($devuelto == "No") {
				...
				$dev = '<td class="text-center"><a class="text-danger" target="_blank"  href="recepcion_registrarequipodevuelto.php?id='.$id.'"><b>No</b></a></td>';
			}else if($devuelto == "Si"){
				 $dev = '<td class="text-center text-info">'. $devuelto .'</td>';
			}else{
				$dev = NULL;
			}
			//Output
