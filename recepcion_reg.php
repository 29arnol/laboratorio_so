...
/*if ($id){
    ...
    $cont = "SELECT *, COUNT(motivo_evaluacion) AS Total FROM datos_basicos 
        WHERE datos_basicos.motivo_evaluacion = 'Ingreso' AND datos_basicos.numero_documento=".$id." ";
      $qcont = mysqli_query($conexion,$cont);
        while ($re = mysqli_fetch_array($qcont)){
      $ingreso = $re{'Total'};      
    }
    $cont = "SELECT *, COUNT(motivo_evaluacion) AS Total FROM datos_basicos 
        WHERE datos_basicos.motivo_evaluacion = 'Periodico' AND datos_basicos.numero_documento=".$id." ";
      $qcont = mysqli_query($conexion,$cont);
        while ($re = mysqli_fetch_array($qcont)){
      $periodico = $re{'Total'};     
    }
    $cont = "SELECT datos_basicos.motivo_evaluacion, COUNT(motivo_evaluacion) AS Total FROM datos_basicos  
        WHERE datos_basicos.motivo_evaluacion = 'Retiro' AND datos_basicos.numero_documento=".$id." ";
      $qcont = mysqli_query($conexion,$cont);
        while ($re = mysqli_fetch_array($qcont)){
      $retiro = $re{'Total'};     
    }
    $cont = "SELECT datos_basicos.motivo_evaluacion, COUNT(motivo_evaluacion) AS Total FROM datos_basicos 
        WHERE datos_basicos.motivo_evaluacion = 'Post incapacidad' AND datos_basicos.numero_documento=".$id." ";
    $qcont = mysqli_query($conexion,$cont);
        while ($re = mysqli_fetch_array($qcont)){
      $postincapacidad = $re{'Total'};     
    }
    $cont = "SELECT datos_basicos.motivo_evaluacion, COUNT(motivo_evaluacion) AS Total FROM datos_basicos
        WHERE datos_basicos.motivo_evaluacion = 'Reubicacion' AND datos_basicos.numero_documento=".$id." ";
    $qcont = mysqli_query($conexion,$cont);
        while ($re = mysqli_fetch_array($qcont)){
     $reubicacion = $re{'Total'};      
    }
    $cont = "SELECT datos_basicos.motivo_evaluacion, COUNT(motivo_evaluacion) AS Total FROM datos_basicos 
        WHERE datos_basicos.motivo_evaluacion = 'Reingreso' AND datos_basicos.numero_documento=".$id." ";
    $qcont = mysqli_query($conexion,$cont);
        while ($re = mysqli_fetch_array($qcont)){
      $reingreso = $re{'Total'};      
    }
    $cont = "SELECT datos_basicos.motivo_evaluacion, COUNT(motivo_evaluacion) AS Total FROM datos_basicos
        WHERE datos_basicos.motivo_evaluacion = 'Otros' AND datos_basicos.numero_documento=".$id." ";
    $qcont = mysqli_query($conexion,$cont);
        while ($re = mysqli_fetch_array($qcont)){
      $otros = $re{'Total'};     
    }
    ...
    WHERE numero_documento = ".$id." order by fecha desc limit 1  ";
...
<?php
  ...
  <div class="container">
    <div style="margin-top:-1%; padding-bottom: 2%;" class="row">
      <div class="col-sm-12">
        <div class="centrarr" style="width: 100%; ">
          <ol class="breadcrumb">
            <li style="font-family: verdana; font-size: 15px;"><a href="recepcion.php">Consultar Numero de Documento</a>
            </li>
            <li style="font-family: verdana; font-size: 15px;" class="active">Registro de Paciente</li>
          </ol>
        </div>  
      </div>
    </div>          
            ...
            <tbody>
              ...
              <fieldset>
                <td>
                  ...
                  <div class="text-center">
                  	<label>Ingreso</label> 
                  	<input type="radio" id="1" name="motivoevaluacion" value="Ingreso"> /
                  	<label>Periódico</label> 
                  	<input type="radio" id="2" name="motivoevaluacion" value="Periodico"> /
                  	<label>Retiro</label> 
                  	<input type="radio" id="3" name="motivoevaluacion" value="Retiro"> /
                  	<label>Post Incapacidad</label> 
                  	<input type="radio" id="4" name="motivoevaluacion" value="Post incapacidad"> / 
                  	<label>Reubicacion</label> 
                  	<input type="radio" id="5" name="motivoevaluacion" value="Reubicacion"> /
                  	<label>Reingreso</label> 
                  	<input type="radio" id="6" name="motivoevaluacion" value="Reingreso">
                    <hr>
                    <div class="text-center">	                      
                      <input type="checkbox" id="otro" name="otro" value="Ingreso" onchange="activar()"> 
                      <label>Otro:</label> 
